    ...
    return (
        <ModalEditProductStyle>
            ...
            <form onSubmit={handleSubmit((data) => editProduct({...data, id}))} noValidate>
                <div>
                    <div>
                        <Input value={img} labelName="Foto do brinquedo" id="toysImage" inputType="url" required={true} placeholder="" width="100%" register={register("img")} />
                        {errors.img && <span>{errors.img.message}</span>}
                        <Input value={toy_name} labelName="Nome" id="toyName" inputType="text" required={true} placeholder="" width="100%" register={register("toy_name")} />
                        {errors.toy_name && <span>{errors.toy_name.message}</span>}
                        <div>
                            <div>
                                <fieldset>
                                    <label htmlFor="toyCategory">Categoria</label>
                                    <select defaultValue={category} id="toyCategory" required={true} {...register("category")} >
                  ...
                  <option value="Outros">Outros</option>
                            ...
                            <div>
                                <Input value={`${price.toFixed(2)}`} labelName="Preço" id="toyPrice" inputType="number" required={true} placeholder="R$" width="100%" register={register("price")} />
                                {errors.price && <span>{errors.price.message}</span>}
                            </div>
                        </div>
                    </div>
                    <div>
                        <Input value={marks} labelName="Marca (opcional)" id="toyMarks" inputType="text" required={true} placeholder="" width="100%" register={register("marks")} />
                        {errors.marks && <span>{errors.marks.message}</span>}
                        <fieldset>
                            <label htmlFor="toyDescription">Descrição</label>
                            <textarea defaultValue={description} id="toyDescription" required={true} {...register("description")}>
